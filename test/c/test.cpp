...
BOOST_AUTO_TEST_CASE(ethash_params_init_genesis_check) {
	...
	BOOST_REQUIRE_MESSAGE(full_size + 20 * UBQHASH_MIX_BYTES >= UBQHASH_DATASET_BYTES_INIT,
			"\nfull size + 20*MIX_BYTES: " << full_size + 20 * UBQHASH_MIX_BYTES << "\n"
					<< "should be greater than or equal to: " << UBQHASH_DATASET_BYTES_INIT << "\n");
	BOOST_REQUIRE_MESSAGE(cache_size < UBQHASH_DATASET_BYTES_INIT / 32,
